...
int GAPS_APT_CSEulerRootFindPushP(double *P,double *LocalA,double *DelPHI,double (*LocalDelA)[3],double *pDt,double *pTolerance,double *pCharge,double *pMass);
int GAPS_APT_CSEulerRootFindFlow(double *F,double *P_iter,double *P_last,double *A,double (*DelA)[3],double *DelPHI,double *pDt,double *pCharge,double *pMass);
int GAPS_APT_CSEulerRootFindJacobi(double **J,double *P_iter,double *P_last,double *A,double (*DelA)[3],double *pDt,double *pCharge,double *pMass);
...
int GAPS_APT_Pusher_RCSA_SymEuler(Gaps_APT_Particle *pPtc,Gaps_IO_InputsContainer *pInputs)
{
	...
	GAPS_APT_CSEulerRootFindPushP(CanP3,A3,DelPHI,DelA3, &dT,&Tolerance,pCharge,pMass);
...
//	double normDx=0;
	...
	for(i=0;i<3;i++)
	{
		Dx[i]=(dT)*(CanP3[i]-pCharge[0]*A3[i])*gamma_Inv;
		pX4[i+1]+=Dx[i];
//		normDx+=pow(Dx[i],2);
	}
	pX4[0]+=dT;
...
int GAPS_APT_CSEulerRootFindPushP(double *P,double *LocalA,double *DelPHI,double (*LocalDelA)[3],double *pDt,double *pTolerance,double *pCharge,double *pMass)
{
	...
	do
	{
		...
		GAPS_APT_CSEulerRootFindFlow(F,P_iter,P_last,LocalA,LocalDelA,DelPHI,pDt,pCharge,pMass);

		tolF=sqrt(F[0]*F[0]+F[1]*F[1]+F[2]*F[2]);
		GAPS_APT_CSEulerRootFindJacobi(Jacobi,P_iter,P_last,LocalA,LocalDelA,pDt,pCharge,pMass);
...
int GAPS_APT_CSEulerRootFindJacobi(double **J,double *P_iter,double *P_last,double *A,double (*DelA)[3],double *pDt,double *pCharge,double *pMass)
{
	...
	for(i=0;i<3;i++)
	{
		for(j=0;j<3;j++)
		{
			...
			J[i][j]=Delta(i,j)+dT*ChargeMass*(sumP-DelA[j][i]*gamma_rev);		
		}
	}
	return 0;
}
