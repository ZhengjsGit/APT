...
int GAPS_APT_Field_HairerRandomWalk_flexEB(double *pTensor,double *pSpaceTime4,int Order,Gaps_IO_InputsContainer *pInputs)
{
	...
	if(-1 == Order)
	{
		...
		{
			pTensor[3] = (scaleB*(yy - zz))/2.;
			pTensor[4] = (scaleB*(xx + zz))/2.;
	...
	if(1 == Order)
	{
		...
		pTensor[1] = (scaleB*(-pow(yy,2) + pow(zz,2)))/4.;
		pTensor[2] = (scaleB*(-pow(xx,2) + pow(zz,2)))/4.;
		pTensor[3] = (scaleB*(-pow(xx,2) + pow(yy,2)))/4.;
	}
	if(2 == Order)
	{
		pTensor[0] = 0;
		...
		pTensor[7] = -(scaleB*xx)/2.;
		pTensor[8] = scaleE*pow(yy,2)*(-3 + 4*yy);
		pTensor[9] = -(scaleB*yy)/2.;
		pTensor[10] = 0;
		pTensor[11] = (scaleB*yy)/2.;
		pTensor[12] = 4*scaleE*pow(zz,3);
		pTensor[13] = (scaleB*zz)/2.;
		pTensor[14] = (scaleB*zz)/2.;
		pTensor[15] = 0;
	}
	if(3 == Order)
	{
	...
	if(MaxOrder<Order)
	{
		fprintf(stderr,"ERROR: In function GAPS_APT_Field_HairerRandomWalk_flexEB. This field function does NOT support tensors order larger than %d.\n",MaxOrder);
	}
	return 0;
}
