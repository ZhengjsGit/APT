...
int GAPS_APT_Field_Henon_Heiles(double *pTensor,double *pSpaceTime4,int Order,Gaps_IO_InputsContainer *pInputs)
{
	...
	if(-1 == Order)
	{
		...
		{
			pTensor[0] = -(xx*(1 + 2*lamb*yy));
			pTensor[1] = -yy + lamb*(-pow(xx,2) + pow(yy,2));
			pTensor[2] = 0;
		}
	...
	if(1 == Order)
	{
		pTensor[0] = (pow(yy,2)*(3 - 2*lamb*yy))/6. + pow(xx,2)*(0.5 + lamb*yy);
	...
	if(MaxOrder<Order)
	{
		fprintf(stderr,"ERROR: In function GAPS_APT_Field_Henon_Heiles. This field function does NOT support tensors order larger than %d.\n",MaxOrder);
	}
	return 0;
}
